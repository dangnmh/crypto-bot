...
// calcSpreadPct computes the bid-ask spread as a percentage of the bid price.
// Returns 0 if bestBid is non-positive.
...
func (o *CycleOrchestrator) consumeTopic(ctx context.Context, topic string, handler func(*message.Message)) {
	...
	if err != nil {
		o.deps.Log.Error("Failed to subscribe to topic", slog.String("topic", topic), slog.Any("error", err))
		return
	}
	go func() {
		for {
...
func (o *CycleOrchestrator) initKlines(ctx context.Context) {
	...
	if err != nil {
		o.deps.Log.Warn("🟡 Failed to fetch initial klines", slog.Any("error", err))
		return
	}
	if len(apiKlines) > 20 {
...
func (o *CycleOrchestrator) waitUntil(ctx context.Context, target time.Time) {
	if d := o.deps.Clock.Until(target); d > 0 {
		o.deps.Log.Debug("⏱️ wait", slog.Time("target", target), slog.Duration("wait", d))
...
// persistCycleRecord builds and persists the cycle record at end-of-cycle.
// Called via defer in Run(), so it executes before bus.Close().
func (o *CycleOrchestrator) persistCycleRecord(ctx context.Context) {
	if o.recorder == nil || o.deps.CycleRecorder == nil {
		return
	}
	...
	cfgJSON, _ := json.Marshal(toTradeConfig(o.cfg))
	record := o.recorder.Build(
		o.cfg.Symbol,
		o.cfg.FundingReversion.TakeProfitPct,
		o.cfg.FundingReversion.StopLossPct,
		cfgJSON,
		entries,
	)
	if err := o.deps.CycleRecorder.Record(ctx, record); err != nil {
		o.deps.Log.Error("🔴 Failed to persist cycle record", slog.Any("error", err))
	} else {
		o.deps.Log.Info("📝 Cycle record persisted", slog.String("outcome", string(record.Outcome)))
	}
}
