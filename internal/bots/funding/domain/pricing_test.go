...
func TestPrepareDynamicPricing(t *testing.T) {
	...
	t.Run("calculates TP and SL from ATR and FR", func(t *testing.T) {
		...
		// tp = (0.1*10 + 2.0*2) / 100 = 5.0 / 100 = 0.05
		// sl = max(0.1*5, 2.0*1.5) / 100 = 3.0 / 100 = 0.03
		if !almostEqual(c.Config.FundingReversion.TakeProfitPct, 0.05, 1e-9) {
			t.Errorf("TP: got %v, want 0.05", c.Config.FundingReversion.TakeProfitPct)
		}
		if !almostEqual(c.Config.FundingReversion.StopLossPct, 0.03, 1e-9) {
			t.Errorf("SL: got %v, want 0.03", c.Config.FundingReversion.StopLossPct)
		}
	})
