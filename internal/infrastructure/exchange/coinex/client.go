...
// Client is the CoinEx REST API client for public market scanning.
type Client struct {
...
// NewClient creates a new CoinEx client.
func NewClient(httpClient *http.Client, baseURL string, _ config.LoggingConfig) *Client {
	logger := slog.Default().With("component", "exchange").With("exchange", "coinex")
