...
func (c *Client) getRawContractDetails(ctx context.Context) ([]kucoinContract, error) {
	body, err := c.base.Request(ctx, http.MethodGet, "/api/v1/contracts/active", nil, nil, false)
	if err != nil {
		return nil, err
	}
	return kucoin.ParseResponse[[]kucoinContract](body, "contract_details")
}

func (c *Client) getRawTickers(ctx context.Context) ([]kucoinTicker, error) {
	body, err := c.base.Request(ctx, http.MethodGet, "/api/v1/allTickers", nil, nil, false)
	if err != nil {
		return nil, err
	}
	return kucoin.ParseResponse[[]kucoinTicker](body, "tickers")
}

func (c *Client) getRawTickerSingle(ctx context.Context, symbol string) (*kucoinSingleTicker, error) {
	params := map[string]string{paramSymbol: symbol}
	body, err := c.base.Request(ctx, http.MethodGet, "/api/v1/ticker", params, nil, false)
	if err != nil {
		return nil, err
	}
	res, err := kucoin.ParseResponse[kucoinSingleTicker](body, "ticker_single")
	if err != nil {
		return nil, err
	}
	return &res, nil
...
// GetContractDetails returns specifications for all active contracts.
...
func (c *Client) getTickerSingle(ctx context.Context, symbol string) ([]exchange.Ticker, error) {
	raw, err := c.getRawTickerSingle(ctx, symbol)
	if err != nil {
		return nil, err
	}
	...
	var vol, amt float64
	cList, err := c.getRawContractDetails(ctx)
	if err == nil {
		for i := range cList {
...
// GetTickers returns ticker data for all contracts or a single symbol.
func (c *Client) GetTickers(ctx context.Context, symbol string) ([]exchange.Ticker, error) {
	if symbol != "" {
		return c.getTickerSingle(ctx, symbol)
	}
...
func (c *Client) GetFundingRates(ctx context.Context, symbols []string) ([]exchange.FundingRateResult, error) {
	...
	contracts, err := c.getRawContractDetails(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, sym := range symbols {
		...
		if !exists {
			c.base.Logger().WarnContext(ctx, "Symbol not found in active contracts", slog.String("symbol", sym))
			continue
		}
...
// GetDepth retrieves the current full Level 2 depth snapshot for a symbol.
func (c *Client) GetDepth(ctx context.Context, symbol string) (*domain.OrderBook, error) {
	params := map[string]string{paramSymbol: symbol}
	body, err := c.base.Request(ctx, http.MethodGet, "/api/v1/level2/snapshot", params, nil, false)
	if err != nil {
		return nil, fmt.Errorf("kucoin get depth for %s: %w", symbol, err)
	}

	data, err := kucoin.ParseResponse[kucoinDepthRawData](body, "depth_snapshot")
	if err != nil {
...
// GetDepthCommits retrieves part orderbook depth data via /api/v1/level2/depth{size}.
func (c *Client) GetDepthCommits(ctx context.Context, symbol string, limit int) ([]exchange.DepthCommit, error) {
	...
	data, err := kucoin.ParseResponse[kucoinDepthRawData](body, "depth_commits")
	if err != nil {
