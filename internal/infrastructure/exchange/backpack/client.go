package backpack

import (
	"context"
	"encoding/json"
	"fmt"
	"io"
	"log/slog"
	"net/http"
	"strings"
	"sync"

	"crypto-bot/internal/infrastructure/config"
	"crypto-bot/internal/infrastructure/exchange"
	"crypto-bot/pkg/httpclient"
	"crypto-bot/pkg/xjson"
)

// Client is the Backpack Exchange public REST API client for scanner integrations.
type Client struct {
	httpClient *http.Client
	baseURL    string
	apiKey     string
	apiSecret  string
	logger     *slog.Logger
}

type backpackTicker struct {
	Symbol      string       `json:"symbol"`
	LastPrice   xjson.Number `json:"lastPrice"`
	QuoteVolume xjson.Number `json:"quoteVolume"`
}

type backpackMarkPrice struct {
	Symbol               string       `json:"symbol"`
	MarkPrice            xjson.Number `json:"markPrice"`
	FundingRate          xjson.Number `json:"fundingRate"`
	NextFundingTimestamp int64        `json:"nextFundingTimestamp"`
}

// NewClient creates a new Backpack client.
func NewClient(httpClient *http.Client, baseURL, apiKey, apiSecret string, logCfg config.LoggingConfig) *Client {
	logger := slog.Default().With("component", "exchange").With("exchange", "backpack")
	if baseURL == "" {
		baseURL = "https://api.backpack.exchange/api/v1"
	}
	var clientCopy http.Client
	if httpClient != nil {
		clientCopy = *httpClient
	}
	if clientCopy.Transport == nil {
		clientCopy.Transport = http.DefaultTransport
	}
	clientCopy.Transport = httpclient.WrapWithRequestID(clientCopy.Transport)

	return &Client{
		httpClient: &clientCopy,
		baseURL:    strings.TrimRight(baseURL, "/"),
		apiKey:     apiKey,
		apiSecret:  apiSecret,
		logger:     logger,
	}
}

func (c *Client) request(ctx context.Context, method, path string) ([]byte, error) {
	req, err := http.NewRequestWithContext(ctx, method, c.baseURL+path, http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("create request: %w", err)
	}

	resp, err := c.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("HTTP request: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read response body: %w", err)
	}

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("HTTP error status %d: %s", resp.StatusCode, string(body))
	}

	return body, nil
}

// GetPotentialFundingSymbols fetches all perpetual contracts, their tickers, and estimated funding rates.
func (c *Client) GetPotentialFundingSymbols(
	ctx context.Context,
	minVol24h, maxVol24h float64,
	whitelist []string,
	blacklist []string,
) ([]exchange.PotentialFundingResult, error) {
	tickers, markPrices, err := c.fetchMarketData(ctx)
	if err != nil {
		return nil, err
	}

	return c.filterAndCombine(tickers, markPrices, minVol24h, maxVol24h, whitelist, blacklist), nil
}

func (c *Client) fetchMarketData(ctx context.Context) ([]backpackTicker, []backpackMarkPrice, error) {
	var (
		tickerData []backpackTicker
		priceData  []backpackMarkPrice
		errTicker  error
		errPrice   error
		wg         sync.WaitGroup
	)

	wg.Add(2)
	go func() {
		defer wg.Done()
		var body []byte
		body, errTicker = c.request(ctx, http.MethodGet, "/tickers")
		if errTicker != nil {
			return
		}
		if err := json.Unmarshal(body, &tickerData); err != nil {
			errTicker = fmt.Errorf("unmarshal tickers: %w", err)
		}
	}()

	go func() {
		defer wg.Done()
		var body []byte
		body, errPrice = c.request(ctx, http.MethodGet, "/markPrices")
		if errPrice != nil {
			return
		}
		if err := json.Unmarshal(body, &priceData); err != nil {
			errPrice = fmt.Errorf("unmarshal markPrices: %w", err)
		}
	}()

	wg.Wait()

	if errTicker != nil {
		return nil, nil, errTicker
	}
	if errPrice != nil {
		return nil, nil, errPrice
	}

	return tickerData, priceData, nil
}

func (c *Client) filterAndCombine(
	tickers []backpackTicker,
	markPrices []backpackMarkPrice,
	minVol24h, maxVol24h float64,
	whitelist, blacklist []string,
) []exchange.PotentialFundingResult {
	tickerMap := make(map[string]backpackTicker)
	for _, item := range tickers {
		tickerMap[strings.ToUpper(item.Symbol)] = item
	}

	whitelistMap := make(map[string]bool)
	for _, sym := range whitelist {
		whitelistMap[strings.ToUpper(sym)] = true
	}
	blacklistMap := make(map[string]bool)
	for _, sym := range blacklist {
		blacklistMap[strings.ToUpper(sym)] = true
	}

	var results []exchange.PotentialFundingResult
	for _, pricing := range markPrices {
		symbolUpper := strings.ToUpper(pricing.Symbol)

		if blacklistMap[symbolUpper] {
			continue
		}
		if len(whitelistMap) > 0 && !whitelistMap[symbolUpper] {
			continue
		}

		ticker, found := tickerMap[symbolUpper]
		if !found {
			continue
		}

		lastPrice := xjson.ToFloat64(ticker.LastPrice)
		volumeUSDT := xjson.ToFloat64(ticker.QuoteVolume)

		if volumeUSDT < minVol24h {
			continue
		}
		if maxVol24h > 0 && volumeUSDT > maxVol24h {
			continue
		}

		rateVal := xjson.ToFloat64(pricing.FundingRate)

		results = append(results, exchange.PotentialFundingResult{
			Symbol:     symbolUpper,
			Rate:       rateVal,
			SettleTime: pricing.NextFundingTimestamp,
			Volume24h:  volumeUSDT,
			Price:      lastPrice,
		})
	}

	return results
}

func (c *Client) GetTopGainer(_ context.Context, _ exchange.TopGainerRequest) ([]exchange.TopGainerResult, error) {
	return nil, nil
}
