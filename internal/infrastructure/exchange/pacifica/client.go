...
// Client is the Pacifica public REST API client.
type Client struct {
...
// NewClient creates a new Pacifica client.
func NewClient(httpClient *http.Client, baseURL string, logCfg config.LoggingConfig) *Client {
	logger := slog.Default().With("component", "exchange").With("exchange", "pacifica")
	if baseURL == "" {
