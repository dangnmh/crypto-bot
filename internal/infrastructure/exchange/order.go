...
func (c *Client) CreateOrder(ctx context.Context, req SubmitOrderRequest) (string, error) {
	body, err := c.PostCtx(ctx, "/api/v1/private/order/create", req)
	if err != nil {
		return "", err
	}

	var resp APIResponse[CreateOrderResponse]
...
func (c *Client) CreateTrackOrder(ctx context.Context, req SubmitTrackOrderRequest) (string, error) {
...
func (c *Client) GetOrder(ctx context.Context, orderID string) (*OrderInfo, error) {
	...
	var resp APIResponse[OrderInfo]
	if err := json.Unmarshal(body, &resp); err != nil {
