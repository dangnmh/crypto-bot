...
// Client is the Extended Exchange public REST API client.
type Client struct {
...
// NewClient creates a new Extended Exchange client.
func NewClient(httpClient *http.Client, baseURL string, logCfg config.LoggingConfig) *Client {
	logger := slog.Default().With("component", "exchange").With("exchange", "extended")
	if baseURL == "" {
