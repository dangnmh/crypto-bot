...
// Client is the HitBTC public REST API client for scanner integrations.
type Client struct {
...
// NewClient creates a new HitBTC client.
func NewClient(httpClient *http.Client, baseURL, apiKey, apiSecret string, logCfg config.LoggingConfig) *Client {
	logger := slog.Default().With("component", "exchange").With("exchange", "hitbtc")
	if baseURL == "" {
		baseURL = "https://api.hitbtc.com/api/3/public"
	}
...
func (c *Client) fetchMarketData(ctx context.Context) (map[string]hitbtcTicker, map[string]hitbtcFuturesInfo, error) {
	var (
		tickerData  map[string]hitbtcTicker
		futuresData map[string]hitbtcFuturesInfo
		errTicker   error
		errFutures  error
		wg          sync.WaitGroup
	)
...
func (c *Client) filterAndCombine(
	...
	futuresInfo map[string]hitbtcFuturesInfo,
	minVol24h, maxVol24h float64,
