...
// Client is the FMFW.io Exchange public REST API client for scanner integrations.
type Client struct {
...
func (c *Client) fetchMarketData(ctx context.Context) (map[string]fmfwTicker, map[string]fmfwFuturesInfo, error) {
	var (
		tickerData  map[string]fmfwTicker
		futuresData map[string]fmfwFuturesInfo
		errTicker   error
		errFutures  error
		wg          sync.WaitGroup
	)
...
func (c *Client) filterAndCombine(
	tickers map[string]fmfwTicker,
	futuresInfo map[string]fmfwFuturesInfo,
	minVol24h, maxVol24h float64,
