...
|---|---|---|---|
| TrackOrder/close failure có thể để unmanaged position | Guarded in code | Nếu TrackOrder placement fail, bot gọi exact-leg `ClosePosition(symbol, close_side, deal_vol, positionMode)` bằng context mới không bị cancel. Nếu exact close fail, bot fallback `CloseAllPositions(symbol)`. Nếu fallback close cũng fail, journal ghi `critical_close_failed`, publish flow error, và abort cycle thay vì publish `position_closed` giả. | Có thể thêm retry/backoff hoặc symbol-level disable khi `disableSymbolAfterCriticalCloseFailure` được wiring đầy đủ. |
