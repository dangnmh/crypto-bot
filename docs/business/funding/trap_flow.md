...
**Hệ quả cho Trap:**
- Wall neo Trap ở T+50ms có thể bị rút khi sóng chính chạy → giá rơi qua Trap → kẹt đáy.
- OB chỉ tin cậy cho **slippage IOC** (tức thì), KHÔNG tin cậy cho Trap placement.

**Biện pháp đã implement:**
- Trap depth dựa vào **FR × multiplier** (thống kê), không phải wall position.
- Wall detection chỉ dùng làm **safety cap** (giảm depth nếu wall quá sát).
- Trailing bật ngay (`activationPct = 0`) để thoát nhanh nếu sóng yếu.

### Khi KHÔNG NÊN Đặt Trap (Đã Implement)

| Tình huống | Lý do | Hành động |
|---|---|---|
| Không tìm thấy wall rõ ràng | Không biết giá dội ở đâu | Fallback về Static Trap |
...
> Xem thêm: [backlog.md §3, §5](backlog.md) — biện pháp chưa implement: verify wall runtime, Imbalance Ratio filter, position size Trap < Reversion.
